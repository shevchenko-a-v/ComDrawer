	...
	private:
		...
		*/
		...
		std::vector<RECT> getRectsRecursive(Node *node, RECT baseRect)
		{
			...
			int ySplit = points_[node->idx][node->axis];
			RECT first, second;
			first.left = baseRect.left;
			first.right = splitByX ? xSplit : baseRect.right;
			first.top = baseRect.top;
			first.bottom = !splitByX ? ySplit : baseRect.bottom;
			second.left = splitByX ? xSplit : baseRect.left;
